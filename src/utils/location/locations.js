import { slugify } from "~/utils/string/slug"
const locations = {
  "districts": [
    {
      "wards": [
        {
          "id": 26734,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Định"]
        },
        {
          "id": 26737,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Đa Kao"]
        },
        {
          "id": 26740,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bến Nghé"]
        },
        {
          "id": 26743,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bến Thành"]
        },
        {
          "id": 26746,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Nguyễn Thái Bình"]
        },
        {
          "id": 26749,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phạm Ngũ Lão"]
        },
        {
          "id": 26752,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Cầu Ông Lãnh"]
        },
        {
          "id": 26755,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Cô Giang"]
        },
        {
          "id": 26758,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Nguyễn Cư Trinh"]
        },
        {
          "id": 26761,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Cầu Kho"]
        }
      ],
      "id": 760,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["1"]
    },
    {
      "wards": [
        {
          "id": 26764,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thạnh Xuân"]
        },
        {
          "id": 26767,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thạnh Lộc"]
        },
        {
          "id": 26770,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hiệp Thành"]
        },
        {
          "id": 26773,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thới An"]
        },
        {
          "id": 26776,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Chánh Hiệp"]
        },
        {
          "id": 26779,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Phú Đông"]
        },
        {
          "id": 26782,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thới Hiệp"]
        },
        {
          "id": 26785,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Trung Mỹ Tây"]
        },
        {
          "id": 26787,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Hưng Thuận"]
        },
        {
          "id": 26788,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Đông Hưng Thuận"]
        },
        {
          "id": 26791,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thới Nhất"]
        }
      ],
      "id": 761,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["12"]
    },
    {
      "wards": [
        {
          "id": 26794,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Linh Xuân"]
        },
        {
          "id": 26797,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Chiểu"]
        },
        {
          "id": 26800,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Linh Trung"]
        },
        {
          "id": 26803,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tam Bình"]
        },
        {
          "id": 26806,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tam Phú"]
        },
        {
          "id": 26809,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hiệp Bình Phước"]
        },
        {
          "id": 26812,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hiệp Bình Chánh"]
        },
        {
          "id": 26815,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Linh Chiểu"]
        },
        {
          "id": 26818,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Linh Tây"]
        },
        {
          "id": 26821,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Linh Đông"]
        },
        {
          "id": 26824,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Thọ"]
        },
        {
          "id": 26827,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Trường Thọ"]
        }
      ],
      "id": 762,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Thủ Đức"]
    },
    {
      "wards": [
        {
          "id": 26830,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Long Bình"]
        },
        {
          "id": 26833,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Long Thạnh Mỹ"]
        },
        {
          "id": 26836,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Phú"]
        },
        {
          "id": 26839,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hiệp Phú"]
        },
        {
          "id": 26842,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tăng Nhơn Phú A"]
        },
        {
          "id": 26845,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tăng Nhơn Phú B"]
        },
        {
          "id": 26848,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phước Long B"]
        },
        {
          "id": 26851,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phước Long A"]
        },
        {
          "id": 26854,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Trường Thạnh"]
        },
        {
          "id": 26857,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Long Phước"]
        },
        {
          "id": 26860,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Long Trường"]
        },
        {
          "id": 26863,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phước Bình"]
        },
        {
          "id": 26866,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Hữu"]
        }
      ],
      "id": 763,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["9"]
    },
    {
      "wards": [
        {
          "id": 26869,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 26872,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 26875,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["17"]
        },
        {
          "id": 26876,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["6"]
        },
        {
          "id": 26878,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["16"]
        },
        {
          "id": 26881,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 26882,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 26884,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 26887,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 26890,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 26893,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 26896,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 26897,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["9"]
        },
        {
          "id": 26898,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["8"]
        },
        {
          "id": 26899,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 26902,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        }
      ],
      "id": 764,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Gò Vấp"]
    },
    {
      "wards": [
        {
          "id": 26905,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 26908,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 26911,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["27"]
        },
        {
          "id": 26914,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["26"]
        },
        {
          "id": 26917,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 26920,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["25"]
        },
        {
          "id": 26923,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 26926,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 26929,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["24"]
        },
        {
          "id": 26932,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 26935,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 26938,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 26941,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 26944,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 26947,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 26950,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["17"]
        },
        {
          "id": 26953,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["21"]
        },
        {
          "id": 26956,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["22"]
        },
        {
          "id": 26959,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["19"]
        },
        {
          "id": 26962,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["28"]
        }
      ],
      "id": 765,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Bình Thạnh"]
    },
    {
      "wards": [
        {
          "id": 26965,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 26968,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 26971,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 26974,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 26977,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 26980,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 26983,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 26986,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 26989,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 26992,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 26995,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 26998,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27001,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27004,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27007,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        }
      ],
      "id": 766,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Tân Bình"]
    },
    {
      "wards": [
        {
          "id": 27010,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Sơn Nhì"]
        },
        {
          "id": 27013,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tây Thạnh"]
        },
        {
          "id": 27016,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Sơn Kỳ"]
        },
        {
          "id": 27019,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Quý"]
        },
        {
          "id": 27022,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thành"]
        },
        {
          "id": 27025,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Thọ Hòa"]
        },
        {
          "id": 27028,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Thạnh"]
        },
        {
          "id": 27031,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Trung"]
        },
        {
          "id": 27034,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hòa Thạnh"]
        },
        {
          "id": 27037,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Hiệp Tân"]
        },
        {
          "id": 27040,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thới Hòa"]
        }
      ],
      "id": 767,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Tân Phú"]
    },
    {
      "wards": [
        {
          "id": 27043,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27046,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27049,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27052,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27055,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27058,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27061,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27064,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27067,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27070,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27073,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27076,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["17"]
        },
        {
          "id": 27079,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27082,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27085,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        }
      ],
      "id": 768,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Phú Nhuận"]
    },
    {
      "wards": [
        {
          "id": 27088,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thảo Điền"]
        },
        {
          "id": 27091,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Phú"]
        },
        {
          "id": 27094,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình An"]
        },
        {
          "id": 27097,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Trưng Đông"]
        },
        {
          "id": 27100,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Trưng Tây"]
        },
        {
          "id": 27103,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Khánh"]
        },
        {
          "id": 27106,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Khánh"]
        },
        {
          "id": 27109,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Cát Lái"]
        },
        {
          "id": 27112,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thạnh Mỹ Lợi"]
        },
        {
          "id": 27115,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Lợi Đông"]
        },
        {
          "id": 27118,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Thủ Thiêm"]
        }
      ],
      "id": 769,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["2"]
    },
    {
      "wards": [
        {
          "id": 27121,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27124,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27127,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27130,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27133,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27136,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27139,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27142,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27145,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27148,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27151,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27154,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27157,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27160,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        }
      ],
      "id": 770,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["3"]
    },
    {
      "wards": [
        {
          "id": 27163,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27166,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27169,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27172,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27175,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27178,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27181,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27184,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27187,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27190,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27193,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27196,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27199,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27202,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27205,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        }
      ],
      "id": 771,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["10"]
    },
    {
      "wards": [
        {
          "id": 27208,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27211,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27214,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27217,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27220,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27223,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27226,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27229,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27232,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27235,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27238,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27241,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27244,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27247,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27250,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27253,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["16"]
        }
      ],
      "id": 772,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["11"]
    },
    {
      "wards": [
        {
          "id": 27256,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27259,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27262,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27265,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27268,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27271,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27274,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27277,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["18"]
        },
        {
          "id": 27280,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27283,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27286,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27289,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["16"]
        },
        {
          "id": 27292,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27295,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27298,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        }
      ],
      "id": 773,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["4"]
    },
    {
      "wards": [
        {
          "id": 27301,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27304,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27307,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27310,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27313,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27316,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27319,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27322,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27325,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27328,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27331,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27334,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27337,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27340,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27343,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        }
      ],
      "id": 774,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["5"]
    },
    {
      "wards": [
        {
          "id": 27346,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27349,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27352,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27355,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27358,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27361,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27364,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27367,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27370,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27373,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27376,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27379,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27382,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        },
        {
          "id": 27385,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        }
      ],
      "id": 775,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["6"]
    },
    {
      "wards": [
        {
          "id": 27388,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["08"]
        },
        {
          "id": 27391,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["02"]
        },
        {
          "id": 27394,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["01"]
        },
        {
          "id": 27397,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["03"]
        },
        {
          "id": 27400,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["11"]
        },
        {
          "id": 27403,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["09"]
        },
        {
          "id": 27406,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["10"]
        },
        {
          "id": 27409,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["04"]
        },
        {
          "id": 27412,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["13"]
        },
        {
          "id": 27415,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["12"]
        },
        {
          "id": 27418,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["05"]
        },
        {
          "id": 27421,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["14"]
        },
        {
          "id": 27424,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["06"]
        },
        {
          "id": 27427,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["15"]
        },
        {
          "id": 27430,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["16"]
        },
        {
          "id": 27433,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["07"]
        }
      ],
      "id": 776,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["8"]
    },
    {
      "wards": [
        {
          "id": 27436,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Hưng Hòa"]
        },
        {
          "id": 27439,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Hưng Hoà A"]
        },
        {
          "id": 27442,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Hưng Hoà B"]
        },
        {
          "id": 27445,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Trị Đông"]
        },
        {
          "id": 27448,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Trị Đông A"]
        },
        {
          "id": 27451,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Trị Đông B"]
        },
        {
          "id": 27454,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Tạo"]
        },
        {
          "id": 27457,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Tạo A"]
        },
        {
          "id": 27460,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Lạc"]
        },
        {
          "id": 27463,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["An Lạc A"]
        }
      ],
      "id": 777,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["Bình Tân"]
    },
    {
      "wards": [
        {
          "id": 27466,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thuận Đông"]
        },
        {
          "id": 27469,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Thuận Tây"]
        },
        {
          "id": 27472,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Kiểng"]
        },
        {
          "id": 27475,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Hưng"]
        },
        {
          "id": 27478,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Bình Thuận"]
        },
        {
          "id": 27481,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Quy"]
        },
        {
          "id": 27484,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Thuận"]
        },
        {
          "id": 27487,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Phú"]
        },
        {
          "id": 27490,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Tân Phong"]
        },
        {
          "id": 27493,
          "type": {
            "name": "Phường",
            "slug": "phuong"
          },
          "names": ["Phú Mỹ"]
        }
      ],
      "id": 778,
      "type": {
        "name": "Quận",
        "slug": "quan"
      },
      "names": ["7"]
    },
    {
      "wards": [
        {
          "id": 27496,
          "type": {
            "name": "Thị trấn",
            "slug": "thi-tran"
          },
          "names": ["Củ Chi"]
        },
        {
          "id": 27499,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phú Mỹ Hưng"]
        },
        {
          "id": 27502,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["An Phú"]
        },
        {
          "id": 27505,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Trung Lập Thượng"]
        },
        {
          "id": 27508,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["An Nhơn Tây"]
        },
        {
          "id": 27511,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Nhuận Đức"]
        },
        {
          "id": 27514,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phạm Văn Cội"]
        },
        {
          "id": 27517,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phú Hòa Đông"]
        },
        {
          "id": 27520,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Trung Lập Hạ"]
        },
        {
          "id": 27523,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Trung An"]
        },
        {
          "id": 27526,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phước Thạnh"]
        },
        {
          "id": 27529,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phước Hiệp"]
        },
        {
          "id": 27532,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân An Hội"]
        },
        {
          "id": 27535,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phước Vĩnh An"]
        },
        {
          "id": 27538,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Thái Mỹ"]
        },
        {
          "id": 27541,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Thạnh Tây"]
        },
        {
          "id": 27544,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Hòa Phú"]
        },
        {
          "id": 27547,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Thạnh Đông"]
        },
        {
          "id": 27550,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bình Mỹ"]
        },
        {
          "id": 27553,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Phú Trung"]
        },
        {
          "id": 27556,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Thông Hội"]
        }
      ],
      "id": 783,
      "type": {
        "name": "Huyện",
        "slug": "huyen"
      },
      "names": ["Củ Chi"]
    },
    {
      "wards": [
        {
          "id": 27559,
          "type": {
            "name": "Thị trấn",
            "slug": "thi-tran"
          },
          "names": ["Hóc Môn"]
        },
        {
          "id": 27562,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Hiệp"]
        },
        {
          "id": 27565,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Nhị Bình"]
        },
        {
          "id": 27568,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Đông Thạnh"]
        },
        {
          "id": 27571,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Thới Nhì"]
        },
        {
          "id": 27574,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Thới Tam Thôn"]
        },
        {
          "id": 27577,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Xuân Thới Sơn"]
        },
        {
          "id": 27580,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Xuân"]
        },
        {
          "id": 27583,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Xuân Thới Đông"]
        },
        {
          "id": 27586,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Trung Chánh"]
        },
        {
          "id": 27589,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Xuân Thới Thượng"]
        },
        {
          "id": 27592,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bà Điểm"]
        }
      ],
      "id": 784,
      "type": {
        "name": "Huyện",
        "slug": "huyen"
      },
      "names": ["Hóc Môn"]
    },
    {
      "wards": [
        {
          "id": 27595,
          "type": {
            "name": "Thị trấn",
            "slug": "thi-tran"
          },
          "names": ["Tân Túc"]
        },
        {
          "id": 27598,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phạm Văn Hai"]
        },
        {
          "id": 27601,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Vĩnh Lộc A"]
        },
        {
          "id": 27604,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Vĩnh Lộc B"]
        },
        {
          "id": 27607,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bình Lợi"]
        },
        {
          "id": 27610,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Lê Minh Xuân"]
        },
        {
          "id": 27613,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Nhựt"]
        },
        {
          "id": 27616,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Kiên"]
        },
        {
          "id": 27619,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bình Hưng"]
        },
        {
          "id": 27622,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phong Phú"]
        },
        {
          "id": 27625,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["An Phú Tây"]
        },
        {
          "id": 27628,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Hưng Long"]
        },
        {
          "id": 27631,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Đa Phước"]
        },
        {
          "id": 27634,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tân Quý Tây"]
        },
        {
          "id": 27637,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bình Chánh"]
        },
        {
          "id": 27640,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Quy Đức"]
        }
      ],
      "id": 785,
      "type": {
        "name": "Huyện",
        "slug": "huyen"
      },
      "names": ["Bình Chánh"]
    },
    {
      "wards": [
        {
          "id": 27643,
          "type": {
            "name": "Thị trấn",
            "slug": "thi-tran"
          },
          "names": ["Nhà Bè"]
        },
        {
          "id": 27646,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phước Kiển"]
        },
        {
          "id": 27649,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phước Lộc"]
        },
        {
          "id": 27652,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Nhơn Đức"]
        },
        {
          "id": 27655,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Phú Xuân"]
        },
        {
          "id": 27658,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Long Thới"]
        },
        {
          "id": 27661,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Hiệp Phước"]
        }
      ],
      "id": 786,
      "type": {
        "name": "Huyện",
        "slug": "huyen"
      },
      "names": ["Nhà Bè"]
    },
    {
      "wards": [
        {
          "id": 27664,
          "type": {
            "name": "Thị trấn",
            "slug": "thi-tran"
          },
          "names": ["Cần Thạnh"]
        },
        {
          "id": 27667,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Bình Khánh"]
        },
        {
          "id": 27670,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Tam Thôn Hiệp"]
        },
        {
          "id": 27673,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["An Thới Đông"]
        },
        {
          "id": 27676,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Thạnh An"]
        },
        {
          "id": 27679,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Long Hòa"]
        },
        {
          "id": 27682,
          "type": {
            "name": "Xã",
            "slug": "xa"
          },
          "names": ["Lý Nhơn"]
        }
      ],
      "id": 787,
      "type": {
        "name": "Huyện",
        "slug": "huyen"
      },
      "names": ["Cần Giờ"]
    }
  ],
  "id": 79,
  "type": {
    "name": "Thành phố",
    "slug": "thanh-pho"
  },
  "names": ["Hồ Chí Minh", "tphcm", "Sài Gòn", "HCM"]
}

const districts = {
  "quan1": "Quận 1",
  "quan2": "Quận 2",
  "quan3": "Quận 3",
  "quan4": "Quận 4",
  "quan5": "Quận 5",
  "quan6": "Quận 6",
  "quan7": "Quận 7",
  "quan8": "Quận 8",
  "quan9": "Quận 9",
  "quan10": "Quận 10",
  "quan11": "Quận 11",
  "quan12": "Quận 12",
  "govap": "Quận Gò Vấp",
  "tanbinh": "Quận Tân Bình",
  "tanphu": "Quận Tân Phú",
  "binhthanh": "Quận Bình Thạnh",
  "phunhuan": "Quận Phú Nhuận",
  "thuduc": "Quận Thủ Đức",
  "binhtan": "Quận Bình Tân",
  "binhchanh": "Bình Chánh",
  "cuchi": "Củ Chi",
  "hocmon": "Hóc Môn",
  "nhabe": "Nhà Bè",
  "cangio": "Cần Giờ",
  "khac": "Quận/Huyện khác"
}


const standardize_district = (locatioName) => {
  const slugText = slugify(locatioName)
  const keys_district = Object.keys(districts)
  const district = keys_district.find(x => slugText.indexOf(x) !== -1)
  if (district) {
    return districts[district]
  }
  return null
}

const standardize_ward = (locationName) => {
  const slugText = slugify(locationName)
  const district = standardize_district(locationName)
  const wards = locations.districts.find(x => slugify(`${x.type.name} ${x.names[0]}`) === slugify(district)).wards
  const ward = wards.find(x => slugText.indexOf(slugify(x.names[0])) !== -1)?.names[0]
  if (ward) {
    return ward
  }
  return null
}

export { standardize_district, standardize_ward }